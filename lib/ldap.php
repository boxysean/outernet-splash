...
class LDAP {
    ...
    public function bindAdmin($pass = NULL) {
    ...
    public function addUserEntry($username, $password, $email="", $phone="") {
        $info = array();
        $info["cn"] = $username;
        $info["sn"] = $username;
        $info["mail"] = array($username . '@' . EMAIL_DOMAIN);

        // LDAP cannot have empty length strings
        if (isset($email) && strlen($email) > 0) {
            array_push($info["mail"], $email);
        }
        ...
        return ldap_add($this->ds, "cn=$username, " . LDAP_PEOPLE_DOMAIN, $info);
    }
}
